...
import {
  ...
  validateElements,
} from '../components/OrderFormFunctions';
import {
  updateOrder,
  getAllCustomers,
  getAllEmployees,
...
export async function getServerSideProps() {
  const { findAllOrders } = await getAllOrders(
    'id price customerId employeeId orderTreatingEmployeeId recieverId message extraInfo productInfo dateOfDelivery orderDate includeDelivery cardType orderState paymentMethod'
  );
  const { findAllCustomers } = await getAllCustomers(
    'id clientName firstName lastName city phoneNumber email postalCode streetName houseNumber'
  );
...
async function handleFormSubmit({
  ...
  let missingInput = validateElements(
    false,
    document.querySelectorAll('input[type=text]')
  );
  missingInput = validateElements(
    missingInput,
  ...
  missingInput = validateElements(missingInput, [
  ...
  let includeDeliveryCosts = document.querySelector(
    'input[name="includeDeliveryCosts"]:checked'
  ).value;
  updateTotalPriceField();
  ...
  let receiver = await updateCustomer(
    orderObj.recieverId,
  ...
  let order = await updateOrder(
    ...
    parseInt(document.getElementById('processEmployee').value),
    receiverObj.id,
    date,
    price,
    ...
    cardSelect.value === 'card-free'
      ? 'BASIC_CARD'
      : cardSelect.value === 'card-ribbon'
      ? 'RIBBON'
      : cardSelect.value === 'card-wish'
      ? 'SPECIAL_CARD'
      : 'NONE'
  );
  if (order.error) {
...
async function handleDelete({ orderObj }) {
  let orderDelete = await deleteOrder(orderObj.id);
  if (orderDelete.error) {
    alert(orderDelete.error.message);
  } else {
    //Redirect to orderOverview if the Order was successfully Deleted.
    window.location.replace('/orderOverview');
  }
}
...
export default function ViewOrder({
  ...
  findAllEmployees.map((f) => {
    ...
    if (intID === order.orderTreatingEmployeeId) {
      assignedEmployee = f;
    }
  });
  ...
  if (order.includeDelivery) {
    if (receiver.city.toLowerCase() === 'hoogeveen') {
      price -= 4.5;
    } else {
      price -= 5.95;
    }
          ...
          >
            <div className={`border-[1px]flex-col w-[45%] ml-[2%]`}>
              ...
              >
                ...
                <div className={`flex flex-col mt-[3%]`}>
                  <label htmlFor="customerPhoneNumber">Telefoonnummer*</label>
                  <input
                    required
                    className={`border-[1px] border-gray-500 h-[25px] w-[100%]`}
                    type="text"
                ...
                <div className={`flex flex-col lg:flex-row justify-between`}>
                  ...
                  <div className={`flex flex-col mt-[3%] w-[95px]`}>
                    ...
                    >
                      <option
                        selected={order.paymentMethod === 'PIN'}
                        value="pin"
                      >
                        Pin
                      </option>
                      <option
                        selected={order.paymentMethod === 'BY_INVOICE'}
                        value="by_invoice"
                      >
                        Factuur
                      </option>
            ...
            >
              ...
              >
                ...
                <div className={`flex flex-col  mt-[3%]`}>
                  <label htmlFor="receiverPhoneNumber">Telefoonnummer*</label>
                  <input
                    required
                    className={`border-[1px] border-gray-500 h-[25px] w-[100%]`}
                    type="text"
          ...
          >
            <div className={`border-[1px]flex-col w-[45%] ml-[2%] mt-[50px]`}>
              ...
              <formHtml>
                ...
                <RedButton onClick={handleDelete}>Order verwijderen</RedButton>
              </formHtml>
            </div>
            <div className={`flex-col w-[45%] mr-[2%] mt-[110px]`}>
              <div
                id={'cardSelectContainer'}
                className={`flex flex-col md:flex-row md:ml-[0px] ml-[5%]`}
              >
              ...
              >
                ...
                >
                  <option
                    selected={order.orderState === 'IN_PROGRESS'}
                    value={'IN_PROGRESS'}
                  >
                    Gelevert maar niet thuis
                  </option>
                  <option
                    selected={order.orderState === 'OPEN'}
                    value={'OPEN'}
                  >
                    Open
                  </option>
                  <option
                    selected={order.orderState === 'DELIVERED'}
                    value={'DELIVERED'}
                  >
                    Verzonden
                  </option>
                  <option
                    selected={order.orderState === 'CLOSED'}
                    value={'CLOSED'}
                  >
                    Voltooid
                  </option>
                ...
                >
                  {findAllEmployees.map((f) => {
                    return (
                      <option
                        selected={f.id === assignedEmployee.id}
                        value={f.id}
                      >
                        {f.name}
                      </option>
              ...
              >
                <BlueButton onClick={printCardLabel}>
                  Tekst kaartje uitprinten
                </BlueButton>
                <BlueButton onClick={printAdressLabel}>
                  Adres uitprinten
                </BlueButton>
              </div>
            </div>
