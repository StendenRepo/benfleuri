...
export async function updateOrder(
  ...
  if (!['OPEN', 'CLOSED', 'IN_PROGRESS', 'DELIVERED'].includes(orderState)) {
    return {
      error: {
        message: 'The gegeven order status is ongeldig (' + orderState + ').',
      },
    };
  }
  ...
  const query = gql`
    mutation UpdateOrder(
      ...
      updateOrder(
        ...
        orderTreatingEmployeeId: $orderTreatingEmployeeId
  ...
  let variables = {
    updateOrderId: orderIDInt,
    customer: customerId,
    employee: employeeId,
    orderTreatingEmployee: assignedEmployee,
    reciever: recieverId,
    price: price,
    paymentMethod: paymentMethod,
    extraInfo: extraInfo,
...
export async function addOrder(
  ...
  let variables = {
    ...
    includeDelivery: includeDelivery,
    cardType: cardType,
    orderTreatingEmployeeId: employeeId,
  };
  const data = await request(
    'http://localhost:3000/api/graphql',
    query,
 ...
 * Deletes order.
 *
 * Returns the id of the deleted Order if successful or an error message if it failed.
 *
 * @param orderID
 *
 * @returns {Promise<{error: {message: string}}|{props: {deleteOrder}}>}
 */
 ...
 */
export async function addCustomerIfNotExists(
  firstName,
  lastName,
  phoneNumber,
  city,
  ...
  if (postalCode.length > 6) {
    return {
      error: {
        message: 'De gegeven postcode voor ' + firstName + ' is ongeldig.',
      },
    };
  }
  ...
  if (houseNumber.length > 6) {
    return {
      error: {
        message: 'Het gegeven huisnummer voor ' + firstName + ' is ongeldig.',
      },
    };
  }

  let customers = await getAllCustomers(
    'id firstName lastName city postalCode'
  );
  let exists = false;
  let id = -1;
  ...
  let data = await request('http://localhost:3000/api/graphql', query, {
    firstName: firstName,
    lastName: lastName,
    phoneNumber: phoneNumber,
    city: city,
